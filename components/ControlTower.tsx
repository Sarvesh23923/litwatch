  ...
  return (
    <section id="control-tower" ref={rootRef} className="border-y border-border bg-lavender-50 py-24 lg:py-32">
      <div className="mx-auto max-w-[1360px] px-6 lg:px-10">
        ...
        >
          <div className="hidden grid-cols-[1.4fr_1.4fr_1fr_1fr] gap-3 border-b border-border px-5 py-3 text-[11px] font-medium tracking-[0.08em] text-muted sm:grid">
            <span>Client</span>
          ...
          <div ref={listRef} className="divide-y divide-border">
            {filtered.map((n) => (
              <div
                key={n.client}
                data-tower="row"
                className="flex flex-col gap-1.5 px-5 py-3.5 sm:grid sm:grid-cols-[1.4fr_1.4fr_1fr_1fr] sm:items-center sm:gap-3"
              >
                <div className="flex items-center justify-between gap-3 sm:contents">
                  <span className="flex min-w-0 items-center gap-2 truncate text-[13.5px] font-medium text-ink">
                    {n.urgent && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />}
                    {n.client}
                  </span>
                  <span className={`inline-flex w-fit shrink-0 items-center rounded-[3px] px-2.5 py-1 text-[11px] font-medium sm:order-4 ${STATUS_STYLE[n.status]}`}>
